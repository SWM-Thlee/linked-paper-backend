...
ECR_REPOSITORY_NAME="api_service_image" # e.g., next_production_image
...
docker buildx build --platform linux/amd64 -t $ECR_REPOSITORY_NAME:$IMAGE_TAG --build-arg ACTIVE_PROFILE=prod .
